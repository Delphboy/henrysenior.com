...
> For intermediate layers, choosing (1-_p_) = 0.5 for large networks is ideal. For the input layer, (1-_p_) should be kept about 0.2 or lower. This is because dropping the input data can adversely affect the training. A (1-_p_) > 0.5 is not advised, as it culls more connections without boosting the regularization.
